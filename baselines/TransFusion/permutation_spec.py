...
class RobertaPermutationSpecBuilder(PermutationSpecBuilder):
    """Permutation specification for Roberta-style transformers."""
    ...
    def create_permutation_spec(self) -> PermutationSpec:
        axes_to_perm = {
            "roberta.embeddings.word_embeddings.weight": (None, "P_hidden"),
            "roberta.embeddings.position_embeddings.weight": (None, "P_hidden"),
            # "roberta.embeddings.token_type_embeddings.weight": (None, "P_hidden"),
            "roberta.embeddings.LayerNorm.weight": ("P_hidden",),
            "roberta.embeddings.LayerNorm.bias": ("P_hidden",),
        }

        for layer_idx in range(self.depth):
            block_prefix = f"roberta.encoder.layer.{layer_idx}"
            axes_to_perm.update({
                f"{block_prefix}.attention.self.query.weight": (f"P{layer_idx}_attn", "P_hidden"),
                f"{block_prefix}.attention.self.key.weight": (f"P{layer_idx}_attn", "P_hidden"),
                f"{block_prefix}.attention.self.value.weight": (f"P{layer_idx}_attn", "P_hidden"),
                f"{block_prefix}.attention.self.query.bias": (f"P{layer_idx}_attn",),
                f"{block_prefix}.attention.self.key.bias": (f"P{layer_idx}_attn",),
                f"{block_prefix}.attention.self.value.bias": (f"P{layer_idx}_attn",),
                f"{block_prefix}.attention.output.dense.weight": ("P_hidden", f"P{layer_idx}_attn"),
                f"{block_prefix}.attention.output.dense.bias": ("P_hidden",),
                f"{block_prefix}.attention.output.LayerNorm.weight": ("P_hidden",),
                f"{block_prefix}.attention.output.LayerNorm.bias": ("P_hidden",),
                f"{block_prefix}.intermediate.dense.weight": (f"P{layer_idx}_mlp", "P_hidden"),
                f"{block_prefix}.intermediate.dense.bias": (f"P{layer_idx}_mlp",),
                f"{block_prefix}.output.dense.weight": ("P_hidden", f"P{layer_idx}_mlp"),
                f"{block_prefix}.output.dense.bias": ("P_hidden",),
                f"{block_prefix}.output.LayerNorm.weight": ("P_hidden",),
                f"{block_prefix}.output.LayerNorm.bias": ("P_hidden",),
            })
