...
def pytorch_cos_sim(a: torch.Tensor, b: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    if a.dim() == 1:
        a = a.unsqueeze(0)
    if b.dim() == 1:
        b = b.unsqueeze(0)
            ...
            for query_idx in range(len(self.query_examples)):
                ...
                if self.use_demo and args.demo_filter:
                    ...
                    for support_idx in candidate:
                        sim_score.append((support_idx, pytorch_cos_sim(self.support_emb[support_idx], self.query_emb[query_idx])))
