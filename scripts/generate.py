...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default=os.path.join(CACHE_DIR, "datasets/original"), help="Path to original data")
    parser.add_argument("--dev_ratio", type=float, default=0.1, help="Ratio of dev set size to total training set size")
    parser.add_argument("--max_train_samples", type=int, default=10000, help="Maximum number of training samples per class")

    args = parser.parse_args()
    args.output_dir = os.path.join(CACHE_DIR, "datasets/lm-bff")
    ...
    for seed in args.seed:
        ...
        for task, dataset in datasets.items():
            ...
            setting_dir = os.path.join(task_dir)
            ...
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
                with open(os.path.join(setting_dir, "train.tsv"), "w") as f:
                    ...
                    for label in label_list:
                        num_train_samples = int((1 - dev_ratio) * len(label_list[label]))
                        for line in label_list[label][:min(num_train_samples, max_train_samples)]:
                            f.write(line)
                name = "dev.tsv"
                if task == 'MNLI':
                ...
                with open(os.path.join(setting_dir, name), "w") as f:
                    ...
                    for label in label_list:
                        num_train_samples = int((1 - dev_ratio) * len(label_list[label]))
                        for line in label_list[label][num_train_samples:]:
                            f.write(line)
            else:
                new_train = []
                for label in label_list:
